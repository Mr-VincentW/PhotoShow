...
## 使用方法
　　超简单的——对于**浮图秀**所支持的网站（请参阅[此列表](#%E6%94%AF%E6%8C%81%E7%BD%91%E7%AB%99)），当您鼠标悬停在页面中的某缩略图上时，若浮图秀能够解析其链接，则会在其旁边展示对应的高分辨率大图。
...
　　若您觉得**浮图秀**图片预览浮层遮挡了太多页面内容，不妨试试前两种视图模式（**迷你**或**轻量**）。

　　当高分辨率大图在图片预览浮层中部分展示时（在**自动**或**全景**模式下可能出现），在源缩略图上会显示一个用于指示图片可视区域的遮罩。您可以通过鼠标或快捷键（在**浮图秀**弹窗中有具体描述）在图片预览浮层中卷动查看图片，此时缩略图上的可视区范围将同步移动。此功能是为查看图片细节或超长图片（例如通过图片发布的文章等）而设计的。不过，请留意：快捷键的操作方向与图片在图片预览浮层中的卷动方向相一致，而非可视区指示范围的移动方向——诚然，在图片经过旋转后，之二者可能并不相同。
...
　　**浮图秀**弹窗中提供了更多设置内容，在浏览器工具栏中点击**浮图秀**图标即可看到：
 ...
 - **设置激活模式**：每次鼠标停在页面中某个缩略图上时都弹出高分辨率大图可能有点烦，通过此设置项您可以为触发图片预览浮层添加一个辅助按键——仅在您需要时按下此键来查看大图。
 - **启闭图标显示**：如果您对享受看图时的纯净视觉盛宴有极高要求，此设置项赋予您给蜷缩在图片预览浮层右下角令人“恼火”的**浮图秀**图标致命一击的权力。
 - **启闭投影显示**：无独有偶，若您也受不了图片预览浮层与源缩略图之间的阴影，干掉它！
 - **查看热键列表**：若您碰巧忘记了那些热键或其操作定义，来这儿就对了。
 - **让一切更美好**：如您有意见建议或绝妙想法，欢迎反馈给我们。当然了，若能得君与亲朋好友们分享**浮图秀**，不胜感激。

　　对了，所有的设置项都会随浏览器账户数据自动同步（如果得到授权），这是浏览器的行为。（更多说明请参阅[协议许可](#%E5%8D%8F%E8%AE%AE%E8%AE%B8%E5%8F%AF)一节。）
...
　　同时，让**浮图秀**“支持所有网站”的计划也已提上日程，更多信息，请参阅[项目进展](#%E9%A1%B9%E7%9B%AE%E8%BF%9B%E5%B1%95)一节。
...
## 项目进展
　　**浮图秀**通过持续迭代优化功能并扩展网站支持范围，所以可能您已经发现其更新频繁。
...
　　另一个亟需帮助的的地方是：**浮图秀**的用户界面需要被翻译成更多语言（包括优化目前已有的语言包），以帮助世界上更多的人。`i18n` 文件早已准备妥当，每一个翻译项都附有详细的说明。您愿意提供帮助吗？
...
　　**浮图秀**仅工作于浏览器本地环境，插件安装时所需的权限仅用于图片的展示和下载。

　　插件自身及其所有开发者、参与者并不收集您的任何个人数据，也不对您因使用浮图秀造成的任何损失、个人信息泄露或因非法下载、使用任何网站资源（包括但不限于图片）所造成的纠纷及后果承担责任。安装使用浮图秀即视为您接受上述条款。
