...
*（祝：浮图秀始终被良好维护并持续更新着，但本 repo 内的代码已于版本 4.36.0 后停止更新。）*
<br />
<br />
...
**浮图秀**的所有开发者、参与者不对您因使用浮图秀造成的任何损失、个人信息泄露或因非法下载、使用任何网站资源（包括但不限于图片）所造成的纠纷及后果承担责任。在任何情况下，我们都不对您因使用浮图秀而可能产生的任何损坏、罚没、损失或费用负责。

**浮图秀**仅在浏览器本地工作。插件安装时所需的权限仅用于图片的展示和下载，请放心使用。从 3.0 版本起，插件要求“存取您在所有网站的数据”，以便于后续插件增加所支持的网站时，您不会再被插件所需权限提升的提示打扰。插件下载图片需要“下载文件”权限，但并不追踪和查看您的下载历史。

自版本 4.50.0（于 2024 年 11 月 4 日发布）起，**浮图秀**开始**匿名**采集其基本使用行为有关的数据，以改进用户体验和功能；这不包含任何个人数据或可用于识别用户身份的信息，确保您的隐私得到保护。

我们保留随时更改本隐私政策而无需单独通知您的权利。任何修改后的隐私政策都将发布在相关网站上。本隐私政策最后一次更新是在 2024 年 11 月 4 日，取代了此前适用的任何早期版本。
