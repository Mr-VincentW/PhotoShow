...
- **启闭随处可用**：浮图秀对其未知的网站默认提供基本支持——当图片的原始尺寸大于其显示尺寸时，展示原始图片；您可以在此启用/关闭该行为。
- **设置激活模式**：每次鼠标停在页面中某个缩略图上时都弹出高分辨率大图可能有点烦，通过此设置项您可以为触发图片预览浮层添加一个辅助按键——仅在您需要时按下此键来查看大图。
- **启闭激活豁免**：当缩略图尺寸大于视窗面积的 25%时，允许禁用大图弹窗。
- **启闭尺寸显示**：如果您需要关注高清图片的原始尺寸，此设置允许您在图片预览浮层右下角显示相关信息。
- **启闭投影显示**：如果您受不了图片预览浮层与源缩略图之间的阴影，干掉它！
- **启闭加载状态**：允许关闭图片加载状态的提示气泡。
- **启闭过渡动画**：允许禁用图片预览窗的各种过渡动画效果。
- **启闭右键菜单**：允许禁用右键菜单项。
...
插件的所有开发者、参与者并不收集您的任何个人数据，也不对您因使用浮图秀造成的任何损失、个人信息泄露或因非法下载、使用任何网站资源（包括但不限于图片）所造成的纠纷及后果承担责任。安装使用浮图秀即视为您接受上述条款。

我们保留随时更改本隐私政策而无需单独通知您的权利。任何修改后的隐私政策都将发布在相关网站上。本隐私政策最后一次更新是在 2020 年 1 月 24 日，取代了此前适用的任何早期版本。
