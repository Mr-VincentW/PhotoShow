...
3. **旋转 & 翻转图片：** 修正方向问题：
   ...
   > :bulb: 小技巧：
   >
   > - 复制、下载或在新标签页中打开图片时，会保留对图片应用的旋转和翻转。
...
- **自动（A）：** 大图浮窗自适应所选的可用位置，最大化图片显示尺寸。
- **适应（F）：** 大图浮窗自适应所选的可用位置，确保整张图片完整显示，无需滚动。
- **全景（P）：** 大图浮窗以原始尺寸显示图片，并按需填充所选的可用位置。
- **轻量（L）：** 大图浮窗自适应所选的可用位置，且使图片的短边不超过 800 像素。
- **迷你（M）：** 大图浮窗自适应所选的可用位置，且使图片的短边不超过 500 像素。
- **小巧（T）：** 大图浮窗自适应所选的可用位置，且使图片的短边不超过 500 像素。

> :information_source: 注：
>
> 除“**适应**”外的视图模式可能会按需启用图片滚动或导航（例如：对于较大或超宽、超高图片）。此时，大图浮窗将显示在缩略图周围，以支持这些交互。
...
> - 关于**高级文件命名替换符**的更多信息，请参阅[使用高级文件名替换符和替换符修饰器](./ADVANCED_FILENAME_PLACEHOLDERS--zh-cn.md)。
...
- **为什么启用 PhotoShow 后，“NVIDIA RTX VSR” 会失效？如何修复？**  
  ...
  1. 打开 **PhotoShow 设置（选项页面）**
  2. 启用：

     - **工具** → **其他设置** → “**简化视觉效果以降低性能影响**”（4.87.0 之前版本）或
     - **外观** → **浮窗样式** → “**简化视觉效果以降低性能影响**”（自 4.87.0 起）或
     - **工具** → **性能优化** → “**简化浮窗视觉效果**”（自 4.91.0 起）
