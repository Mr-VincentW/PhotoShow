...
> [!NOTE]
> 本仓库源码自迁移到 Extension Manifest V3 后不再更新，但浮图秀始终被积极维护并持续改进着。
...
- **自动（A）：** 大图浮窗自适应使图片最大化显示的可用已选位置，超宽或超高图片可滚动查看。
- **适应（F）：** 与“自动”模式类似但始终完整显示整张图片，不应用滚动或导航。
- **全景（P）：** 在大图浮窗中以原始尺寸显示图片并按需启用导航。
- **轻量（L）：** 与“自动”模式类似且使大图浮窗短边不超过 800 像素。
- **迷你（M）：** 与“自动”模式类似且使大图浮窗短边不超过 500 像素。
- **小巧（T）：** 与“自动”模式类似且使大图浮窗短边不超过 300 像素。
...
- **缩略图类型** 及 **触发豁免**：控制哪些缩略图会触发大图浮窗。
- **浮窗配色** 及 **浮窗样式**：为大图浮窗选择 `浅色` 或 `深色` 主题及外观样式。
- **过渡动画**：启用流畅动画，或简化/关闭动画。
- **视图模式**：控制大图浮窗显示尺寸及交互模式。
- **浮窗定位**：默认在缩略图旁边，你也可以选择 `重叠` 以允许全屏显示。
- **图片信息显示**：显示图片说明、尺寸、格式或文件大小。
- **新标签页打开方式**：选择新开图片标签页在前台或后台打开。
- **键盘快捷键**：启用/禁用特定快捷键。
- **图片下载**：使用占位符自定义文件名（例如：`我的图片/<H>/<c>`）。
- **辅助与优化**：额外功能，如标记已查看的图片或启用/禁用右键菜单项。
- **性能优化**：禁用可能占用较多系统资源的功能来提升性能。
...
- **如何全屏查看图片？**  
  在**浮窗定位**中启用 `重叠` 选项。启用后，大图浮窗可以与缩略图重叠，并可占用整个屏幕来显示图片。

  > :information_source: 注：
  >
  > 除“**适应**”以外的**视图模式**可能会在需要时启用图片滚动或导航（例如对于大图或超宽/超高图片）。在这种情况下，大图浮窗会定位在缩略图周围以支持这些交互，因此可能无法扩展至全屏。
