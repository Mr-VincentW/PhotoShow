![浮圖秀海報](resources/banner--zh-tw.png)

<div align="right">
  
:link: [English](README.md) &emsp; :link: [简体中文](README--zh-cn.md)

</div>

# 浮圖秀

**浮圖秀**是一款瀏覽器擴充功能，只需將滑鼠懸停在縮圖或圖片連結上，即可檢視或下載高清圖片。無縫整合於所有您喜愛的網站。

> [!NOTE]
> 本倉庫原始碼自遷移至 Extension Manifest V3 後不再更新，但浮圖秀持續被積極維護與改進。

<br />
<br />

## 目錄

- :rocket: [安裝浮圖秀](#rocket-%E5%AE%89%E8%A3%9D%E6%B5%AE%E5%9C%96%E7%A7%80)
- :fire: [常用功能](#fire-%E5%B8%B8%E7%94%A8%E5%8A%9F%E8%83%BD)
- :gear: [個性化設定](#gear-%E5%80%8B%E6%80%A7%E5%8C%96%E8%A8%AD%E5%AE%9A)
- :question: [常見問題](#question-%E5%B8%B8%E8%A6%8B%E5%95%8F%E9%A1%8C)
- :lady_beetle: [發現 BUG？](#lady_beetle-%E7%99%BC%E7%8F%BE-bug)
- :memo: [條款與隱私政策](#memo-%E6%A2%9D%E6%AC%BE%E8%88%87%E9%9A%B1%E7%A7%81%E6%94%BF%E7%AD%96)
- :speech_balloon: [聯絡作者](#speech_balloon-%E8%81%AF%E7%B5%A1%E4%BD%9C%E8%80%85)

<br />
<br />

## :rocket: 安裝浮圖秀

**浮圖秀**已入駐各大瀏覽器應用商店並成為**推薦擴充功能**！點擊下方安裝：

- <img width="24" align="center" src="resources/logo-chrome.png" alt="Google Chrome" /> [Google Chrome](https://chromewebstore.google.com/detail/photoshow/mgpdnhlllbpncjpgokgfogidhoegebod)
- <img width="24" align="center" src="resources/logo-edge.png" alt="Microsoft Edge" /> [Microsoft Edge](https://microsoftedge.microsoft.com/addons/detail/afdelcfalkgcfelngdclbaijgeaklbjk)
- <img width="24" align="center" src="resources/logo-firefox.png" alt="Mozilla Firefox" /> [Mozilla Firefox](https://addons.mozilla.org/firefox/addon/photoshow/)

<br />
<br />

## :fire: 常用功能

非常簡單——訪問網站，滑鼠懸停在縮圖或圖片連結上，浮圖秀會自動檢測並顯示高畫質圖片。

<p align="center"><img width="640" src="resources/spec__view-images--zh-tw.png" alt="浮圖秀 - 檢視圖片" /></p>
<br />

您還可以：

1. **下載圖片：** 按 `S` 一鍵下載。
   <p align="center"><img width="640" src="resources/spec__download-images--zh-tw.png" alt="浮圖秀 - 下載圖片" /></p>

2. **複製圖片：** 按 `C` 複製圖片以便編輯或貼到聊天。
   <p align="center"><img width="640" src="resources/spec__copy-images--zh-tw.png" alt="浮圖秀 - 複製圖片" /></p>

3. **旋轉 & 翻轉圖片：** 修正方向問題：

   - 旋轉：`Shift` + `Ctrl` + `←` / `→`
   - 翻轉：`Alt` + `Ctrl` + `←` / `→`
   <p align="center"><img width="640" src="resources/spec__rotate-images--zh-tw.png" alt="浮圖秀 - 旋轉與翻轉圖片" /></p>

> [!TIP]
> 複製、下載或在新分頁中開啟圖片時，會保留套用於圖片的旋轉和翻轉。

<br />

浮圖秀提供獨特的**捲動模式**來檢視**超寬**或**超高**圖片。此模式下圖片不會被縮小，而是在大圖浮窗內顯示局部並在縮圖上顯示**導航器**，如同放大鏡效果。移動滑鼠即可檢視整張圖片。

<p align="center"><img width="640" src="resources/spec__view-ultra-wide-or-tall-images--zh-tw.png" alt="浮圖秀 - 檢視超寬/超高圖片" /></p>
<br />

在**全景模式**下，您可以用同樣的導航系統在各方向自由探索。

<p align="center"><img width="640" src="resources/spec__view-image-details--zh-tw.png" alt="浮圖秀 - 檢視圖片細節" /></p>

> [!TIP]
> **導航快捷鍵：**
> - `←` / `→` / `↑` / `↓`：逐像素移動（長按加速）。
> - `Home` / `End`：跳至頂部/底部（超高圖片）。
> - `PgUp` / `PgDn`：按視口高度滾動（超高圖片）。

<br />

### 檢視模式

提供六種模式：

- **自動（A）：** 大圖浮窗自適應使圖片最大化顯示的可用已選位置，超寬或超高圖片可捲動查看。
- **適應（F）：** 與“自動”模式類似但始終完整顯示整張圖片，不應用捲動或導覽。
- **全景（P）：** 在大圖浮窗中以原始尺寸顯示圖片並按需啟用導覽。
- **輕量（L）：** 與“自動”模式類似且使大圖浮窗短邊不超過 800 像素。
- **迷你（M）：** 與“自動”模式類似且使大圖浮窗短邊不超過 500 像素。
- **小巧（T）：** 與“自動”模式類似且使大圖浮窗短邊不超過 300 像素。

> [!NOTE]
> 除「**適應**」外的檢視模式可能會按需啟用圖片捲動或導覽（例如：對於較大或超寬、超高圖片）。此時，大圖浮窗將顯示在縮圖周圍，以支援這些互動。

> [!TIP]
> - 可使用括號中的字母快捷鍵切換檢視模式。
> - 按 `V` 可在最近使用的兩種檢視模式間切換。
> - 上述快捷鍵預設禁用，可在設定中啟用。

<br />
<br />

## :gear: 個性化設定

浮圖秀提供兩級靈活設定：

- **全域設定：** 對所有網站生效（在**擴充選項**頁面）。
- **網站設定：** 僅對單個網站生效（透過工具列彈窗訪問）。

**網站設定**會覆蓋**全域設定**，雙層設計實現靈活控制。

<p align="center"><img width="640" src="resources/spec__settings--zh-tw.png" alt="浮圖秀 - 設定" /></p>
<br />

### 主要選項包括：

- **白名單模式**：全域停用浮圖秀，僅在指定網站透過彈出視窗開啟。
- **觸發模式**：需要按下輔助鍵才能顯示大圖浮窗。
- **縮圖類型** 與 **觸發豁免**：控制哪些縮圖會觸發大圖浮窗。
- **浮窗配色** 與 **浮窗樣式**：為大圖浮窗選擇 `淺色` 或 `深色` 主題及外觀樣式。
- **過渡動畫**：啟用平滑動畫，或簡化/停用。
- **檢視模式**：控制大圖浮窗顯示尺寸及互動模式。
- **浮窗定位**：預設顯示於縮圖旁，你也可以選擇 `重疊` 以允許全螢幕顯示。
- **圖片資訊顯示**：顯示圖片說明、尺寸、格式或檔案大小。
- **新分頁開啟方式**：選擇新圖片分頁在前景或背景開啟。
- **鍵盤快捷鍵**：啟用/停用特定快捷鍵。
- **圖片下載**：使用替換符來自訂檔名（例如：`我的圖片/<H>/<c>`）。
- **輔助與優化**：額外功能，如標記已檢視的圖片，或啟用/停用右鍵選單項目。
- **效能最佳化**：停用可能占用較多系統資源的功能，以提升效能。
- **設定管理**：匯出/匯入設定以便跨裝置使用，或重設全域或網站專屬設定。

<br />

> [!NOTE]
> - 全域設定預設隨瀏覽器帳號同步（可在瀏覽器設定中關閉）。
> - 網站專屬設定因技術限制，僅在本機保存。
> - 匯出或匯入操作同時包含以上兩類設定。

> [!TIP]
> - 當大圖浮窗置中顯示時，超高或超寬圖片不會套用「**捲動模式**」；如需保留此模式，請啟用所有**浮窗定位**選項，則浮圖秀會自動選擇合適的位置，以最大尺寸顯示浮窗並在需要時套用「**捲動模式**」。
> - 檔命名可包含**路徑**，如 `我的圖片/<H>/<c>` → `預設下載資料夾/我的圖片/(網站域名)/(圖片說明)`。
> - 關於**進階檔名稱替代符**的更多資訊，請參閱[使用進階檔名替代符與替代符修飾器](./ADVANCED_FILENAME_PLACEHOLDERS--zh-tw.md)。

<br />
<br />

## :question: 常見問題

- 為什麼啟用 PhotoShow 後，「NVIDIA RTX VSR」會失效？如何修復？  
  這通常是因為 PhotoShow 的部分視覺效果（尤其是背景模糊）對 GPU 負載較高，可能會影響 NVIDIA RTX 影像超解析度（VSR）的正常啟用。當這些效果啟用時，VSR 可能無法如預期運作。

  要解決此問題，請啟用用於簡化視覺效果的選項：

  1. 開啟 **PhotoShow 設定（選項頁面）**
  2. 啟用：

     - **工具** → **其他設定** → 「**簡化視覺效果以降低效能影響**」（4.87.0 之前版本）或
     - **外觀** → **浮窗樣式** → 「**簡化視覺效果以降低效能影響**」（自 4.87.0 起）或
     - **工具** → **效能最佳化** → 「**簡化浮窗視覺效果**」（自 4.91.0 起）

  啟用後，如有需要，請重新整理頁面或重新啟動瀏覽器以確保設定生效。

- **為什麼檔名設定不起作用？**  
  其他擴充功能也可能修改下載檔名，若未生效，請檢查是否有其他擴充功能覆蓋。

- **為什麼每次下載圖片都要我選擇儲存位置？**  
  預設情況下，浮圖秀會自動將圖片儲存到瀏覽器/系統的預設下載資料夾，您可以在**圖片下載**設定中調整此路徑。

  <p align="center"><img width="612" src="resources/spec__image-download-settings--zh-tw.png" alt="浮圖秀 - 圖片下載設定" /></p>

  如果您每次都被要求選擇儲存位置，請嘗試以下步驟：

  - 關閉浮圖秀**圖片下載**設定中的「**每次下載前詢問**」選項。
  - 關閉瀏覽器本身下載設定中的類似「總是詢問」選項。
  - 檢查是否安裝了管理下載的其他擴充功能，並相應調整他們的設定。

- **為什麼浮圖秀好像不能記住我上次儲存圖片的位置？**  
  浮圖秀會記住您在**圖片下載**設定中配置的儲存路徑，而該路徑始終基於瀏覽器/系統的預設下載資料夾。

  如果您在下載時手動選擇了**預設下載資料夾之外**的路徑，就可能覺得「沒有被記住」——出於安全原因，擴充功能不能在之後自動重複使用這些路徑。

  若您希望每次自由選擇儲存位置，可以啟用「**每次下載前詢問**」選項。

- **如何將 WebP 保存為 JPG？**  
  在**圖片下載**設定中選擇 `jpg` 作為檔案副檔名。浮圖秀會自動轉換格式。

- **如何全螢幕檢視圖片？**  
  在**浮窗定位**中啟用 `重疊` 選項。啟用後，大圖浮窗可以與縮圖重疊，並可占用整個螢幕來顯示圖片。

  > :information_source: 注：
  >
  > 除「**適應**」以外的**檢視模式**可能會在需要時啟用圖片捲動或導覽（例如對於大圖或超寬／超高圖片）。在這種情況下，大圖浮窗會定位在縮圖周圍以支援這些互動，因此可能無法擴展至全螢幕。

- **大圖浮窗能否在滑鼠移開後保持顯示？**  
  目前還不行。浮圖秀從一開始即被設計為提供「即用即走」的清爽互動體驗，所以當滑鼠移開時，大圖會自動關閉。未來的更新會加入更多可選項，讓您可以根據需要自行選擇。

- **還有哪些功能在規劃中？**  
  浮圖秀未來將支援：

  - [ ] 滑鼠滾輪縮放
  - [ ] 在圖集/輪播中切換圖片
  - [ ] 自訂快捷鍵
  - [ ] 在大圖浮窗中播放影片

  敬請期待！ :smiley:

<br />
<br />

## :lady_beetle: 發現 BUG？

浮圖秀會定期更新，但在為數百個網站手工適配過程中，仍可能出現偶發問題。

請透過 [Discord 社群](https://discord.com/invite/U7cSS4bDfU)（推薦）、[GitHub Issues](../../../issues) 頁面或郵件回報問題或功能請求。

在建立提案時，請：

- 盡可能提供模板中列出的詳細資訊；
- 建立新提案前請先搜尋現有提案。

<br />
<br />

## :memo: 條款與隱私政策

請參見 [隱私政策與使用條款](https://www.photoshow.cool/terms-zh-tw)。

<br />
<br />

## :speech_balloon: 聯絡作者

:email: [發送郵件](mailto:vincentwang863@gmail.com?subject=%E6%B5%AE%E5%9C%96%E7%A7%80%E4%BD%BF%E7%94%A8%E8%80%85%E5%9B%9E%E9%A5%8B%20-%20GitHub)
