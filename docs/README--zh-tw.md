...
3. **旋轉 & 翻轉圖片：** 修正方向問題：
   ...
   > :bulb: 小技巧：
   >
   > - 複製、下載或在新分頁中開啟圖片時，會保留套用於圖片的旋轉和翻轉。
...
- **自動（A）：** 大圖浮窗自適應所選的可用位置，最大化圖片顯示尺寸。
- **適應（F）：** 大圖浮窗自適應所選的可用位置，確保整張圖片完整顯示，無需捲動。
- **全景（P）：** 大圖浮窗以原始尺寸顯示圖片，並按需填滿所選的可用位置。
- **輕量（L）：** 大圖浮窗自適應所選的可用位置，且使圖片的短邊不超過 800 像素。
- **迷你（M）：** 大圖浮窗自適應所選的可用位置，且使圖片的短邊不超過 500 像素。
- **小巧（T）：** 大圖浮窗自適應所選的可用位置，且使圖片的短邊不超過 300 像素。

> :information_source: 注：
>
> 除「**適應**」外的檢視模式可能會按需啟用圖片捲動或導覽（例如：對於較大或超寬、超高圖片）。此時，大圖浮窗將顯示在縮圖周圍，以支援這些互動。

> :bulb: 小技巧：
>
> - 可使用括號中的字母快捷鍵切換檢視模式。
> - 按 `V` 可在最近使用的兩種檢視模式間切換。
> - 上述快捷鍵預設禁用，可在設定中啟用。
...
- **白名單模式**：全域停用浮圖秀，僅在指定網站透過彈出視窗開啟。
- **觸發模式**：需要按下輔助鍵才能顯示大圖浮窗。
- **縮圖類型** 與 **觸發豁免**：控制哪些縮圖會觸發大圖浮窗。
- **浮窗定位**：預設顯示於縮圖旁，你也可以選擇 `置中` 以允許全螢幕顯示。
- **圖片資訊顯示**：顯示圖片說明、尺寸、格式或檔案大小。
- **新分頁開啟方式**：選擇新圖片分頁在前景或背景開啟。
- **轉場動畫**：啟用平滑動畫，或簡化/停用。
- **浮窗配色**：為大圖浮窗選擇 `淺色` 或 `深色` 主題。
- **鍵盤快捷鍵**：啟用/停用特定快捷鍵。
- **圖片下載**：使用替換符來自訂檔名（例如：`我的圖片/<H>/<c>`）。
- **輔助與優化**：額外功能，如標記已檢視的圖片，或啟用/停用右鍵選單項目。
- **設定管理**：匯出/匯入設定以便跨裝置使用，或重設全域或網站專屬設定。

<br />

> :information_source: 注意：
>
> - 全域設定預設隨瀏覽器帳號同步（可在瀏覽器設定中關閉）。
> - 網站專屬設定因技術限制，僅在本機保存。
> - 匯出或匯入操作同時包含以上兩類設定。

> :bulb: 小技巧：
>
> - 當大圖浮窗置中顯示時，超高或超寬圖片不會套用「**捲動模式**」；如需保留此模式，請啟用所有**浮窗定位**選項，則浮圖秀會自動選擇合適的位置，以最大尺寸顯示浮窗並在需要時套用「**捲動模式**」。
> - 檔命名可包含**路徑**，如 `我的圖片/<H>/<c>` → `預設下載資料夾/我的圖片/(網站域名)/(圖片說明)`。
> - 關於**進階檔名稱替代符**的更多資訊，請參閱[使用進階檔名替代符與替代符修飾器](./ADVANCED_FILENAME_PLACEHOLDERS--zh-tw.md)。
...
- 為什麼啟用 PhotoShow 後，「NVIDIA RTX VSR」會失效？如何修復？  
  這通常是因為 PhotoShow 的部分視覺效果（尤其是背景模糊）對 GPU 負載較高，可能會影響 NVIDIA RTX 影像超解析度（VSR）的正常啟用。當這些效果啟用時，VSR 可能無法如預期運作。

  要解決此問題，請啟用用於簡化視覺效果的選項：

  1. 開啟 **PhotoShow 設定（選項頁面）**
  2. 啟用：

     - **工具** → **其他設定** → 「**簡化視覺效果以降低效能影響**」（4.87.0 之前版本）或
     - **外觀** → **浮窗樣式** → 「**簡化視覺效果以降低效能影響**」（自 4.87.0 起）或
     - **工具** → **效能最佳化** → 「**簡化浮窗視覺效果**」（自 4.91.0 起）

  啟用後，如有需要，請重新整理頁面或重新啟動瀏覽器以確保設定生效。
...
- **大圖浮窗能否在滑鼠移開後保持顯示？**  
  目前還不行。浮圖秀從一開始即被設計為提供「即用即走」的清爽互動體驗，所以當滑鼠移開時，大圖會自動關閉。未來的更新會加入更多可選項，讓您可以根據需要自行選擇。
...
請透過 [Discord 社群](https://discord.com/invite/U7cSS4bDfU)（推薦）、[GitHub Issues](../../../issues) 頁面或郵件回報問題或功能請求。
