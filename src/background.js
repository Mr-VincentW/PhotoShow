...
const websiteConfig = {
  ...
  '(?:web\\.)?500px\\.com': {
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => {
        var link = trigger.is('.link_wrap') ? utils.getBackgroundImgSrc(trigger.next('.top')) : trigger.attr('href') || src,
      ...
      processor: (trigger, src, srcRegExpObj) => {
        ...
        return userId ? type == 'avatar' && utils.cacheImage(userId) || new Promise(resolve => {
          chrome.runtime.sendMessage({
            cmd: 'CROSS_ORIGIN_GET',
            args: {
            ...
            (type == 'avatar' && response.user.userpic_url && utils.cacheImage(userId, response.user.userpic_url) || response.user.cover_url) || ''));
        }) : '';
      }
    }, {
      ...
      processor: (trigger, src, srcRegExpObj) => {
        ...
        return groupId ? utils.cacheImage(groupId) || new Promise(resolve => {
          chrome.runtime.sendMessage({
            cmd: 'CROSS_ORIGIN_GET',
            args: {
            ...
            utils.cacheImage(groupId, response.group.avatars[Object.keys(response.group.avatars).sort((size1, size2) => parseInt(size2) - parseInt(size1))[0]].url) || ''));
        }) : '';
      }
    }, {
  ...
  '(?:www|image)\\.baidu\\.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src) => {
        ...
        return utils.detectImage(src).then(imgInfo => imgInfo.src ||
          utils.detectImage('//timgsa.baidu.com/timg?quality=80&size=b9999_10000&imgtype=0&src=' + encodeURIComponent(src), '', img => img.width * img.height < 50).then(imgInfo => imgInfo.src ||
  ...
  '.+\\.bing\\.com': {
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => {
        ...
        return utils.detectImage(data.murl, defaultSrc).then(imgInfo => imgInfo.src);
      }
    }, {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => {
        if (srcRegExpObj.test(src)) {
          ...
          src = utils.cacheImage(userId) || utils.detectImage(RegExp.$1 + '-original' + RegExp.$2, RegExp.$1 + '-big' + RegExp.$2).then(imgInfo => utils.cacheImage(userId, imgInfo.src));
        } else {
          src = '';
        }
  ...
  'www\\.facebook\\.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => {
        var bgSrc = utils.getBackgroundImgSrc(trigger);
        ...
        .then(() => {
          ...
          var cachedHdImgSrc = fbId && utils.cacheImage(fbId);
        ...
        .catch(() => {
          var cachedHdImgSrc = profileId && utils.cacheImage(profileId);
          return cachedHdImgSrc ? {
            id: profileId,
            src: cachedHdImgSrc
          } : new Promise((resolve, reject) => {
        ...
        .then(hdImgInfo => {
          ...
          utils.cacheImage(hdImgInfo.id, hdImgInfo.src);
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => {
        var apiKey = $('[photoshow-flickr-apikey]').val() || (/"site_key"\s*:\s*"(\w+)"/.test($('script:contains("site_key")').text()) ? RegExp.$1 : ''),
          photoId = srcRegExpObj.test(decodeURIComponent(src) || utils.getBackgroundImgSrc(trigger) || utils.getBackgroundImgSrc(trigger.find('.photo'))) ? RegExp.$1 : '';
  ...
  '(?:.+\\.)?(jd|yhd|tuniu)\\.(?:com|hk)': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) ? utils.detectImage(RegExp.$1 + RegExp.$2, RegExp.$1 + '_w800_h0_c0_t0' + RegExp.$2).then(imgInfo => imgInfo.src) : ''
    }]
  },
  'www\\.kmart\\.com': {
    srcMatching: {
  ...
  'www\\.pinterest(?:\\.(?:com|[a-z]{2}))+': {
    srcMatching: {
      ...
      processor: (trigger, src, srcRegExpObj) => trigger.find('video').attr('poster') || (srcRegExpObj.test(src || utils.getLargestImgSrc(trigger.closest('.pinWrapper').find('img'))) ? (RegExp.$1 + 'originals' + RegExp.$2) : '')
    }
  },
  '(?:.+\\.)?(?:pixiv(?:ision|-bungei)?\\.net|booth\\.pm|vroid\\.com)': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) ? utils.detectImage(RegExp.$1 + 'img-original' + RegExp.$2 + (RegExp.$3 || '_ugoira0') + RegExp.$4, RegExp.$1 + 'img-original' + RegExp.$2 + (RegExp.$3 || '_ugoira0') + '.png').then(imgInfo => imgInfo.src) : ''
    }, {
      selectors: 'img,[style*="background-image"]',
  ...
  'www\\.quora.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src) => trigger.is('.link_overlay') ? utils.getBackgroundImgSrc(trigger.closest('.HyperLinkPreview').find('.hyperlink_image')) : src
    }, {
      ...
      processor: (trigger, src) => {
        ...
        return imgs.length ? (imgs.attr('master_src') || imgs.attr('src') || utils.getBackgroundImgSrc(imgs) || src) : new Promise(resolve => {
          var expandedConObserver = new MutationObserver(mutations => {
            imgs = $(hasEmbededCon ? '.ui_qtext_embed' : 'img.ui_qtext_image', expandedCon);
            if (imgs.length) {
              src = imgs.attr('master_src') || imgs.attr('src') || utils.getBackgroundImgSrc(imgs) || src;
  ...
  '(?:www\\.)?trademe(?:\\.co)?\\.nz': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) || srcRegExpObj.test(utils.getBackgroundImgSrc(trigger.find('.image'))) ? (RegExp.$1 + 'plusw' + RegExp.$2) : ''
    }, {
  ...
  'www\\.(?:torpedo7|1-day)\\.co\\.nz': {
    srcMatching: {
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src || trigger.next('.c-card-product__image-wrap').find('img').attr('src')) ? utils.detectImage(RegExp.$1 + RegExp.$2, RegExp.$1 + '_zoom' + RegExp.$2, img => img.width * img.height <= 1).then(imgInfo => imgInfo.src) : ''
    }
  },
  ...
  '.+\\.tumblr\\.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src ||
        ...
        utils.getBackgroundImgSrc(trigger.parent().find('.post_thumbnail_container,.post-thumbnail-container,.thumbnail_anchor'))
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src ||
        ...
        utils.getBackgroundImgSrc(trigger.parent().find('.post_thumbnail_container,.post-thumbnail-container'))
      ) ? RegExp['$&'] : ''
    }, {
  ...
  '(?:.+\\.)?weibo\\.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) ? utils.detectImage(RegExp.$1 + 'original' + RegExp.$2, RegExp.$1 + 'large' + RegExp.$2, img => img.width == 75 && img.height == 75).then(imgInfo => imgInfo.src) : ''
    }, {
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) ? utils.detectImage(RegExp.$1 + '1' + RegExp.$2, RegExp.$1 + '180x240x75x0x0x1' + RegExp.$2).then(imgInfo => imgInfo.src) : ''
    }, {
  ...
  'www\\.youtube\\.com': {
    ...
    srcMatching: [{
      ...
      processor: (trigger, src, srcRegExpObj) => srcRegExpObj.test(src) ? utils.detectImage(RegExp.$1 + 'hq720' + RegExp.$2, RegExp.$1 + 'hqdefault' + RegExp.$2, img => img.width == 120 && img.height == 90).then(imgInfo => imgInfo.src) : ''
    }, {
...
var photoShow = {
  checkWebsiteState: function(tabUrl) {
    var urlHostname = utils.getUrlHostname(tabUrl);
  ...
  downloadImg: function(imgSrc, tabUrl) {
    imgSrc && chrome.downloads.download({
      url: imgSrc,
      filename: utils.getImgFileName(imgSrc, tabUrl),
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.cmd) {
    ...
    case 'SET_PHOTOSHOW_STATE':    // Args: tabUrl, isPhotoShowEnabled
      var urlHostname = utils.getUrlHostname(request.args.tabUrl);
...
chrome.tabs.query({
  ...
  if (!chrome.runtime.lastError && tabs && tabs.length) {
    photoShow.checkWebsiteState(tabs[0].url);
    photoShow.setWebsiteState(tabs[0].id, tabs[0].url);
  }
});